...
describe("LyricsDisplay", () => {
  ...
  it("renders all language versions by default", () => {
    ...
    expect(
      screen.getByText("בָּרָאיֶה טוּיֶה כּוּצֶ׳ה רַקְסִידַן"),
    ).toBeInTheDocument();
  ...
  it("renders multiple lines sorted by lineNumber", () => {
    ...
    expect(screen.getByText("For dancing in the alley")).toBeInTheDocument();
    expect(
      screen.getByText("For being afraid at the moment of kissing"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("For my sister, your sister, our sisters"),
    ).toBeInTheDocument();
  });
  ...
  it("calls onLineClick with startTime and index when line is clicked", () => {
    ...
    const firstLine = screen
      .getByText("For dancing in the alley")
      .closest("button");
    if (firstLine) {
  ...
  describe("Language Filter", () => {
    it('shows only source language when filter is "source"', () => {
      render(
        <LyricsDisplay
          songId={"test-song-id" as never}
          languageFilter="source"
        />,
        { wrapper: createWrapper() },
      );
      ...
      expect(
        screen.queryByText("Barāye tūye kūche raqsidan"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("בָּרָאיֶה טוּיֶה כּוּצֶ׳ה רַקְסִידַן"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("For dancing in the alley"),
      ).not.toBeInTheDocument();
    });
    ...
    it('shows only Transliteration when filter is "transliteration"', () => {
      ...
      expect(
        screen.getByText("Barāye tūye kūche raqsidan"),
      ).toBeInTheDocument();

      // Other languages should NOT be visible
      expect(
        screen.queryByText("برای توی کوچه رقصیدن"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("בָּרָאיֶה טוּיֶה כּוּצֶ׳ה רַקְסִידַן"),
      ).not.toBeInTheDocument();
  ...
  it("handles lines without Hebrew text gracefully", () => {
    ...
    expect(
      screen.getByText("برای خواهرم خواهرت خواهرامون"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Barāye khāharam khāharet khāharāmūn"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("For my sister, your sister, our sisters"),
    ).toBeInTheDocument();
