...
// Normalize legacy "persian" filter value to "original" (backward compat)
function normalizeLanguageFilter(filter: string | undefined): LanguageFilter {
  if (filter === "persian") return "original";
  if (
    filter === "original" ||
...
function SongPageContent({ songId }: SongPageContentProps) {
  ...
  const [languageFilter, setLanguageFilter] = useState<LanguageFilter>(
    normalizeLanguageFilter(userPreferences?.languageFilter),
  );
          ...
          >
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="text-left min-w-0 flex-1">
                <h1 className="text-sm font-bold truncate iran-gradient">
                  {song.title}
                </h1>
